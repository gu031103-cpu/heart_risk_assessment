...
with st.sidebar:
    ...
    **特征数**: 48 个 (经多方法投票筛选)
    ...
    artifacts_dir = st.text_input(
        ...
        help="该目录下需包含 model_LightGBM.pkl, scaler.pkl, "
             "train_enc_columns.pkl 等训练阶段保存的文件。",
    )
...
def render_result() -> None:
    ...
    with right:
        ...
        st.caption(
            f"📍 临床决策阈值 = **{OPTIMAL_THRESHOLD:.3f}**　|　"
            f"低风险分位 ≤ **{LOW_RISK_QUANTILE:.3f}**　|　"
            f"高风险分位 ≥ **{HIGH_RISK_QUANTILE:.3f}**"
        )
...
def _render_risk_legend(prob: float) -> None:
    ...
    for col, (name, lo, hi, color) in zip(cols, bands):
        ...
        col.markdown(
            ...
            f"{lo:.3f} – {hi:.3f}</div></div>",
            unsafe_allow_html=True,
        )


def _render_contributors(df: pd.DataFrame) -> None:
    """渲染 SHAP top-K 贡献条形图(纯 HTML)。"""
    ...
    for _, row in df.iterrows():
        ...
        color = '#d62728' if shap_val > 0 else '#2ca02c'
        side = 'left' if shap_val > 0 else 'right'
        bar_html = (
            f"<div style='background:#f0f0f0; border-radius:4px; height:18px;"
            f"position:relative; margin-top:4px;'>"
            f"<div style='background:{color}; width:{pct:.1f}%; height:100%;"
            f"border-radius:4px; float:{side};'></div></div>"
        )
